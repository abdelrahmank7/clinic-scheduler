...
function DashboardPage() {
  ...
  // Fetch Range Mode
  const [fetchRangeMode, setFetchRangeMode] = useState("month");
  ...
  // === USE THE CUSTOM HOOK ===
  // This replaces the manual `appointments` state and the big `useEffect` for fetching
  const {
    appointments,
    loading,
    error,
    ...
    resizeAppointment,
    moveAppointment,
  } = useAppointments(calendarRange, selectedLocations);
  ...
  useEffect(() => {
    ...
    const requestsUnsubscribe = onSnapshot(
      ...
      (err) => {
        ...
        setErrorRequests("Failed to load approved requests.");
        setLoadingRequests(false);
      }
    );
  ...
  // --- Helper to update range based on Date and View ---
  const updateRange = (targetDate, targetView) => {
    // If we are in "all" mode, use the wide range
    if (fetchRangeMode === "all") {
      setCalendarRange({
        start: moment().subtract(10, "years").startOf("year").toDate(),
        end: moment().add(10, "years").endOf("year").toDate(),
      });
      return;
    }
    ...
    let start, end;
    if (targetView === "month") {
      start = moment(targetDate).startOf("month").toDate();
      end = moment(targetDate).endOf("month").toDate();
    } else if (targetView === "week") {
      start = moment(targetDate).startOf("week").toDate();
      end = moment(targetDate).endOf("week").toDate();
    } else if (targetView === "day") {
      start = moment(targetDate).startOf("day").toDate();
      end = moment(targetDate).endOf("day").toDate();
    } else {
      start = moment(targetDate).startOf("month").toDate();
      end = moment(targetDate).endOf("month").toDate();
    }
  ...
  // 1. Handle Navigation (Next, Back, Today)
  const handleNavigate = (newDate) => {
    setCalendarDate(newDate);
    updateRange(newDate, calendarView);
  };

  // 2. Handle View Change (Month, Week, Day, Agenda)
  const handleViewChange = (newView) => {
    setCalendarView(newView);
    updateRange(calendarDate, newView);
  };
  ...
  // The helper functions (handleEventResize, handleEventDrop, etc.)
  // are now provided by the hook (resizeAppointment, moveAppointment, etc.)
  // We can wrap them here if needed to match prop signatures, or pass directly.

  const handleEventResize = async ({ event, start, end }) => {
    await resizeAppointment({ event, start, end });
  };

  const handleEventDrop = async ({ event, start, end }) => {
    await moveAppointment({ event, start, end });
  };
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 md:mb-6 border-b pb-4 gap-4 md:gap-0">
          ...
          <div className="flex flex-wrap justify-center md:justify-end items-center gap-2">
            ...
            <Button
              onClick={() => {
                const newRangeMode =
                  fetchRangeMode === "month" ? "all" : "month";
                setFetchRangeMode(newRangeMode);
                // Trigger an update
                if (newRangeMode === "month") {
                  updateRange(calendarDate, calendarView);
                } else {
                  updateRange(calendarDate, calendarView); // Will trigger the 'all' logic in helper
                }
                toast({
                  title:
                    newRangeMode === "all"
                      ? "Fetching All Appointments"
                      : "Fetching Current View",
                  description:
                    newRangeMode === "all"
              ...
              variant={fetchRangeMode === "all" ? "default" : "outline"}
            >
              {fetchRangeMode === "all"
                ? "View Current Range"
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 flex-grow">
          ...
          <div className="md:w-3/4 w-full flex flex-col">
            <CalendarView
              ...
              date={calendarDate}
              // Handlers
              onNavigate={handleNavigate} // Handles Next/Back/Today
              onView={handleViewChange} // Handles Month/Week/Day/Agenda buttons
              onMarkDone={(id) => handleUpdateStatus(id, "done")}
              onMarkMissed={(id) => handleUpdateStatus(id, "missed")}
              onMarkPostponed={(id) => handleUpdateStatus(id, "postponed")}
            />
          </div>
        </div>
      </div>
